...
window.onload = function(){
	...
	var navLi = document.querySelectorAll('.list li');
	...
	for(var i=0; i<navLi.length; i++){
		...
		navLi[i].onmouseover = function(){
			this.style.backgroundColor = '#101d2a';
		};
	...
	for(var i=0; i<mailLi.length; i++){
		mailLi[i].onmouseover = function(){
			if(!this.children[0].children[0].checked){//当前邮件没被选中，则执行
				this.style.backgroundColor = '#f1f6f9';
			}
		};
		mailLi[i].onmouseout = function(){//当前邮件没被选中，则执行
	...
	function deleEle(){
		...
		checkBox = mail.querySelectorAll('.checkbox');//重新获取目前邮件的选中按钮
